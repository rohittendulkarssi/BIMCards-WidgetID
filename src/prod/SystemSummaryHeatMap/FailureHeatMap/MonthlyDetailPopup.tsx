...
const MonthlyDetailPopup: React.FunctionComponent<IProps> = (props) => {
    
    const [TableData, setTableData] = React.useState([]);
    const [Loading, setLoading] = React.useState(false);
    const [StartDate, setStartDate] = React.useState(`${props.DetailOBJ.y}-${fixingDigit((parseInt(props.DetailOBJ.x)+1))}-01`+` 00:00:00`);
    

    React.useEffect(() => {
        setStartDate(`${props.DetailOBJ.y}-${fixingDigit((parseInt(props.DetailOBJ.x)+1))}-01`+` 00:00:00`);
    }, [props]);
    ...
    if(Loading){
        return(
            <div style={{marginTop:"-35px"}}>
                <TitleBar title={`From ${props.DetailOBJ.y}/${fixingDigit((parseInt(props.DetailOBJ.x)+1))}/01`+` 00:00:00 To 
                 ${props.DetailOBJ.y}/${fixingDigit((parseInt(props.DetailOBJ.x)+2))}/01`+` 00:00:00`}>
...
export default MonthlyDetailPopup;